...
def page_replacement(capacity, reference_string):
    ...
    # Initialize an empty frame list, page fault count, page fault flag, and stack for LRU replacement policy
    f, fault, pf = [], 0, 'No'
    stack = []
    ...
    for i in range(len(s)):
        ...
        if s[i] not in f:
            # If there is space in the frame, add the page to the frame and the stack
            if len(f) < capacity:
                f.append(s[i])
                stack.append(s[i])
            # If the frame is full, replace the page using LRU policy
            else:
                x = stack.pop(0)
                f.remove(x)
                f.append(s[i])
            ...
            # If the page is already in the frame, update the stack
            stack.remove(s[i])
            stack.append(s[i])
            pf = 'No'
